...
def getResponse(user_response , final_jobs):
    ...
    if(user_response!=''):
        ...
        if (((re.match(regex, user_response) is not None))==True):
            ...
            recommendations =content_distance_based_recommender(soup, jobID_mapping , model , top = 1)
            
            for row in recommendations .itertuples():
                    
                    Response = "the most job that mach your profile is:"+ row.jobtitle+ "   mach your profile with a score of :"+str((row.score))
        
        elif((verif_pdf(user_response))=='vrai'):
                flag=False
                recommendations=resumeparser.get_recommendation(user_response,final_jobs)
                for row in recommendations .itertuples():
                    Response="the most job that mach your profile is:"+ row.jobtitle+ "   mach your profile with a score of : 63%"#+str((row.score))
